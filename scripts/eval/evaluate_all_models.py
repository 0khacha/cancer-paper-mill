...
def main():
    ...
    for name in ["val", "test", "holdout"]:
        # Original
        with open(f"data/final_v2_period_stripped/cancer_pm_{name}.json", "r", encoding="utf-8") as f:
            splits[f"{name}_orig"] = json.load(f)
        # Normalized
        with open(f"data/final_v2_period_stripped/cancer_pm_{name}_normalized.json", "r", encoding="utf-8") as f:
