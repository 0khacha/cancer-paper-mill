...
## 1. Construction du Jeu de Données CPM-11K (Phase 4)
...
### 1.1. Identification de la source positive (RWDB)
Les exemples positifs (articles issus d'usines à articles) ont été extraits de la base de données Retraction Watch Database (RWDB). Le processus s'est déroulé de la manière suivante :
...
2.  **Filtrage thématique PubMed :** La requête PubMed a été restreinte au domaine oncologique par l'inclusion d'une clause booléenne globale de mots-clés (`cancer OR oncology OR tumor OR tumour OR neoplasm OR carcinoma OR leukemia OR lymphoma OR melanoma OR sarcoma OR metastasis OR malignant OR glioma`).
...
## 2. Protocole Stratifié de Division des Données (Phase 5)
...
## 3. Analyse Exploratoire des Données (EDA) (Phase 6)
...
## 4. Modélisation et Évaluations Lexicales (Phase 7)
...
5.  **Embeddings Généraux (all-MiniLM-L6-v2) :** Extraction de plongements sémantiques généraux de dimension 384 à partir du texte combiné (titre + résumé), suivie d'une régression logistique.
6.  **Embeddings Biomédicaux (PubMedBERT) :** Extraction de plongements sémantiques spécialisés de dimension 768 à partir d'un modèle BERT pré-entraîné sur la littérature scientifique PubMed (`pritamdeka/S-PubMedBert-MS-MARCO`), suivie d'une régression logistique.
...
### 5.2. Analyse qualitative des causes de rétractation (RWDB)
...
2.  **Strate Hindawi (Fraude axée sur le processus) :**
    ...
    *   **61,9 %** des notices font également référence à du contenu assisté par ordinateur (`computer-aided/computer-generated content`).
