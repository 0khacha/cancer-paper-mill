...
Notre modèle de référence, un classifieur linéaire robuste basé sur des sacs de mots pondérés (TF-IDF Combiné, $min\_df=5$, $C=15,0$), atteint une performance globale de premier ordre avec un score F1 global de 54,64 % et un ROC-AUC de 80,85 % sur l'ensemble de validation. Il convient de noter que la configuration exacte du modèle de référence a été reconstruite par alignement sur les métriques publiées plutôt que vérifiée à partir des scripts d'origine, constituant une limite de cette étude. Cependant, une évaluation stratifiée par éditeur révèle une asymétrie de performance majeure : alors que les articles frauduleux de Spandidos (caractérisés par des fraudes d'images grossières habillées de textes stéréotypés) sont détectés avec un F1 de 87,80 %, le modèle échoue face à une prédiction positive naïve sur le sous-ensemble éditorial Hindawi. Une analyse causale qualitative des motifs de rétractation et un test dose-réponse par rééchantillonnage bootstrap suggèrent que les fraudes basées sur la manipulation des processus éditoriaux (évaluation par les pairs biaisée, cartels de citations) ne laissent pas de signal lexical facilement extractible par les représentations testées dans les résumés. Ces résultats sont cohérents avec une évaluation finale sur un ensemble de test indépendant (ROC-AUC de 65,71 % sur Hindawi) et un ensemble de généralisation étanche Hindawi-Holdout de 1 504 articles (F1 de 39,95 % vs 44,39 % pour la baseline triviale, ROC-AUC de 64,18 %). Ce travail suggère les limites de l'analyse textuelle sémantique (au niveau des résumés) pour la détection de la fraude de processus, et ouvre la voie à des approches multi-modales intégrant les métadonnées de publication.
...
| **SciBERT (fine-tuned)** | 52,61% | 59,13% | 55,68% | 84,47% | 58,41% |
...
1. **vs. TF-IDF Baseline :** McNemar $\chi^2 = 76,41$ ($p = 2,31 \times 10^{-18}$, p-ajustée de Holm-Bonferroni = $1,15 \times 10^{-17}$), ce qui confirme une différence hautement significative. Le bootstrap estime une différence de F1 moyenne de $+7,81\%$ avec un intervalle de confiance à 95% de $[4,66\%, 11,08\%]$, excluant largement 0.
2. **vs. Character N-grams :** McNemar $\chi^2 = 0,025$ ($p = 0,873$, p-ajustée = $0,873$), montrant que la répartition des erreurs de classification individuelles n'est pas statistiquement distincte au seuil de $5\%$. Cependant, le bootstrap F1 estime une différence de F1 moyenne de $+8,69\%$ avec un intervalle de confiance à 95% de $[4,20\%, 13,37\%]$, indiquant une amélioration robuste et significative de la métrique F1 globale en faveur du modèle de mots.
3. **vs. Embeddings Biomédicaux (PubMedBERT gelé) :** McNemar $\chi^2 = 28,56$ ($p = 9,10 \times 10^{-8}$, p-ajustée = $3,64 \times 10^{-7}$), confirmant une différence statistiquement significative. Le bootstrap estime une différence de F1 moyenne de $+11,07\%$ avec un intervalle de confiance à 95% de $[6,81\%, 15,74\%]$, démontrant la supériorité statistique du modèle lexical sur les embeddings gelés.
4. **vs. PubMedBERT et SciBERT (fine-tunés) :** Le test de McNemar confirme que la répartition des erreurs de PubMedBERT ($\chi^2 = 1,25$, $p=0,264$, p-ajustée = $0,791$) et de SciBERT ($\chi^2 = 0,59$, $p=0,443$, p-ajustée = $0,887$) n'est pas significativement différente du modèle TF-IDF Combiné. Le bootstrap F1 estime une différence moyenne de $-0,10\%$ pour PubMedBERT (IC 95%: $[-4,42\%, +3,91\%]$) et $-0,93\%$ pour SciBERT (IC 95%: $[-5,30\%, +3,38\%]$). L'intervalle incluant 0 dans les deux cas, les modèles neuronaux ne sont pas statistiquement distinguables du modèle classique.
...
La performance finale des modèles a été mesurée sur l'ensemble de test (`cancer_pm_test.json`), qui n'a été accédé qu'une seule fois. Le seuil de décision a été gelé sur l'ensemble de validation (0,35) avant d'accéder aux ensembles de test et de holdout. Au seuil ainsi figé, les performances globales et par strate se structurent comme suit :
...
| :--- | :---: | :---: | :---: | :---: |
| **TF-IDF Combiné** | 49,45% / 79,60% | 38,98% / 65,78% | 81,25% / 97,16% | 54,61% / 89,21% |
| **PubMedBERT (v2)** | 49,52% / 81,83% | 38,17% / 66,06% | 87,50% / 94,57% | 57,27% / 90,09% |
| **SciBERT (v2)** | 50,96% / 83,83% | 36,52% / 70,42% | 77,42% / 92,89% | 63,11% / 90,75% |

Une comparaison directe avec les résultats de validation montre une baisse modérée de F1 global pour tous les modèles (ex: de 54,64 % à 49,45 % pour TF-IDF), tandis que les ROC-AUC restent extrêmement stables. Ce comportement montre une généralisation robuste de la capacité de tri des modèles, bien que la strate Hindawi reste indétectable pour l'ensemble des approches, qu'elles soient classiques ou neuronales profondes.
...
*   **Métriques du Holdout (seuil = 0,35) :** 
    *   **TF-IDF Combiné (C=15.0, corrigé) :** Précision = 46,18 %, Rappel = 35,20 %, F1 = 39,95 %, ROC-AUC = 64,18 %. *(Les valeurs antérieures Précision = 46,41 %, Rappel = 36,13 %, F1 = 40,63 %, ROC-AUC = 64,47 % correspondaient à la configuration C=10.0 invalidée — NE PAS CITER.)*
...
1. **Manipulation systémique du processus éditorial (réseaux d'évaluation par les pairs ou peer-review rings) :** Dans le cas d'Hindawi, les réseaux d'usines à articles ont massivement exploité la création de numéros spéciaux (*Special Issues*) en corrompant ou en plaçant des éditeurs invités complices. Parce que la validation académique des manuscrits était garantie par cette corruption administrative (les articles étant acceptés d'off### Section 5.8: Erratum Méthodologique : La "Séparation Parfaite" et le Piège des Frontières de Chaînes

**Erratum et Correction de l'Hypothèse Initiale**
Dans la première itération de notre analyse (rapportée initialement comme un succès du modèle profond), les modèles de séquence (PubMedBERT et SciBERT fine-tunés) avaient atteint une séparation parfaite étonnante (AUC et F1 de 100,00 %) sur le "Sous-groupe B" (articles frauduleux de processus pur, sans flag d'IA générative). Nous avions initialement conclu que le modèle profond était parvenu à sur-apprendre une signature stylistique authentique propre à l'usine à articles, là où les modèles classiques (comme TF-IDF) échouaient. 

Cependant, cette conclusion était erronée. Une inspection ultérieure, plus granulaire, des résidus de tokenisation aux frontières des chaînes de caractères a révélé que la séparation parfaite reposait exclusivement sur un artefact de formatage asymétrique, et non sur un style linguistique.

Spécifiquement, 100 % des titres positifs issus de la base de données externe RWDB se terminaient de façon asymétrique sans point (`.`), tandis que plus de 99,9 % des titres négatifs provenant de l'API PubMed se terminaient par un point en raison des conventions de formatage propres à PubMed. L'architecture de fine-tuning conservait cette ponctuation de fin de séquence, apprenant très rapidement à classer le texte uniquement sur la présence/absence du point final.

**Pourquoi la Batterie Diagnostique Initiale a-t-elle Échoué ?**
Notre cascade diagnostique originale comportait quatre contrôles rigoureux (tests de normalisation, audits de chevauchement, tests de mémorisation, et stress-tests sur échantillons négatifs difficiles), qui avaient tous échoué à identifier cette fuite. Cette défaillance s'explique par un angle mort partagé : aucun de ces tests ne contrôlait explicitement le formatage trivial aux limites de la séquence (string boundaries). La normalisation initiale traitait les mots à l'intérieur du texte, mais pas la ponctuation finale. Ceci constitue une leçon méthodologique critique : aucune batterie de tests conçue pour traquer des mécanismes de fuite sémantiques ou de mémorisation complexes ne détectera une anomalie triviale si aucune vérification fondamentale n'examine d'abord l'hygiène de formatage basique des chaînes de caractères. Un résultat "parfait" qui survit à de multiples tests sophistiquéd est souvent le signe que ces tests partagent le même angle mort méthodologique, plutôt que l'indication forte de la validité de l'apprentissage.

**Résultats Corrigés (Dataset v2)**
Nous avons uniformément nettoyé la ponctuation limitrophe (points, espaces, points-virgules) de tous les titres dans nos ensembles (le dataset `v2`). Après ré-entraînement sur ce corpus purifié, la performance des modèles s'est effondrée vers des valeurs réalistes :
- **PubMedBERT** atteint désormais un score **F1 de 54,76 %** et une **AUC de 83,72 %** sur l'ensemble de validation global.
- **SciBERT** obtient des résultats similaires avec un **F1 de 55,68 %** et une **AUC de 84,47 %**.
- Le test de **permutation des labels** (entraîné sur des labels aléatoires pendant 3 époques) a été évalué en calculant l'intervalle de confiance à 95% par rééchantillonnage bootstrap (2 000 itérations) sur les probabilités combinées de **seulement 2 graines aléatoires** (seeds 123 et 456) pour chaque partition. Bien que l'utilisation de seulement 2 graines représente une limite statistique par rapport au bootstrap standard utilisé ailleurs dans l'étude, les résultats montrent des performances proches du hasard (50%), écartant une fuite structurelle :
    - **Validation :** AUC = 53,73 % (IC 95 % : $[50,04 \%, 57,52 \%]$).
    - **Test :** AUC = 54,67 % (IC 95 % : $[50,60 \%, 58,77 \%]$).
    - **Holdout :** AUC = 51,04 % (IC 95 % : $[47,75 \%, 54,22 \%]$).

**Cohérence avec les Conclusions Classiques (Sous-groupe A et B)**
La chute la plus révélatrice se situe au niveau du Sous-groupe B. L'AUC de ces 52 cas "pure fraude" s'est effondrée de 100,00 % à **62,54 %** (PubMedBERT) et **66,44 %** (SciBERT), atterrissant précisément au niveau du plafond de performance classique identifié à la Section 5.3 (où les embeddings figés de PubMedBERT stagnaient autour de ~59-63 % d'AUC sur les Sous-groupes A et B). 

Ce retour au plafond classique est une vérification de cohérence très solide : des modèles différents (linéaires sur embeddings figés vs fine-tuning complet) sur des sous-groupes similaires convergent désormais vers le même plafond de performance algorithmique (AUC de 60-66%). Cela infirme définitivement notre conclusion antérieure d'un "signal de style" appris par les modèles profonds, et réaffirme que la fraude de processus par manipulation du peer-review (sans génération IA évidente) ne laisse pas de signature lexicale extractible. Le plafond de performance pour détecter la fraude de processus uniquement par l'analyse textuelle du résumé demeure bloqué autour de $\sim 55 \%$ de F1 (le TF-IDF Combiné atteignant désormais 54,64 %).
